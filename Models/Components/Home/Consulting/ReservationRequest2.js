...
class ReservationRequest2 extends Component{
    constructor(props) {
        super(props);
        this.state = {
            Processing: false,
        ...
        if (lang == "AR") {
           return (
              <View style={[styles.flex, styles.row, styles.shadow, { width: width, height: 55, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#28B5AF', paddingHorizontal: 18 }]} >
                 ...
                 <Text style={{ color: '#fff', fontSize: 22,fontFamily:'adobe' }}>طلب حجز</Text>
                 <View></View>
              </View>
           )
        } else {
           return (
              <View style={[styles.flex, styles.row, styles.shadow, { width: width, height: 55, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#28B5AF', paddingHorizontal: 18 }]} >
                 <View></View>
                 <Text style={{ color: '#fff', fontSize: 22,fontFamily:'adobe' }}> Reservation request</Text>
                 <TouchableOpacity onPress={() => this.props.navigation.goBack()} >
            ...
            <View style={{width:width,height:height,alignItems:'center',}}>
                <View style={[this.props.Language == "AR" ? styles.row :styles.rowReversed,styles.view,styles.shadow,{padding:3}]}>
                     ...
                     <Icon name="sort-desc" style={{ fontSize: 15,color: '#707070',margin:10 }} />
                        <DatePicker
                         style={{ flex:1}}
                           ...
                           onDateChange={(date) => {
                              this.setState({date})
                              this.props.getTimes(this.state.data.id , date , 1)
                           }}
                        />
                     </View>
                  </View>
                 ...
                 <View style={[this.props.Language == "AR" ? styles.row : styles.rowReversed, styles.shadow, { width:'100%', height: 40, alignItems: 'center',backgroundColor:'#fff',borderRadius:5 }]}>
                   ...
                   textStyle={[this.props.Language == "AR"?styles.right:styles.left,{fontSize: 18, color: this.state.color,paddingHorizontal:'9%',fontFamily:'adobe' }]}
               ...
               <View style={[styles.row, { justifyContent: 'center', alignItems: 'center',paddingHorizontal: 24, }]}>
                  <Item style={[styles.inputFields, styles.shadow, { }]}>
                     <Input
                      ...
                      style={[this.props.Language == "AR" ? styles.right :styles.left ,{ color: '#000',fontSize:17,height:100,textAlignVertical:'top',fontFamily:'adobe' }]}
               ...
               <Text style={[this.props.Language == "AR" ? styles.right :styles.left,{ color: '#BDBDBD', fontSize: 20,fontFamily:'adobe',
                 ...
                 <Text style={[this.props.Language == "AR" ? styles.right :styles.left,{ color: '#BDBDBD', fontSize: 20,fontFamily:'adobe',
                   ...
                   {this.props.Language == "AR" ? "ستنتهى الأستشارة تلقائيا بعد 20 دقيقة" : "The counseling will end automatically after 20 minutes"}
                 </Text>
               ...
               <LinearGradient colors={['#47D9D3', '#3D7371']} style={[styles.button,styles.shadow,]}>
                  <TouchableOpacity 
                  onPress={() => {
                     const { date , timeName , description } = this.state
                        if(date && timeName && description ){
                            ...
                            AsyncStorage.setItem('MakeRequest2', JSON.stringify(obj))
                            this.setState({date :'' , time:'' ,description:''})
                            this.props.navigation.navigate('ConsultationPay') 
                        }else{
...
const mapStateToProps = state => {
 ...
 // redux
 export default connect(mapStateToProps,{ SetLoading , getTimes })(ReservationRequest2)

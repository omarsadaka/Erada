...
class TherapeuticSessions extends Component{
    constructor(props) {
        super(props);
        this.state = {
            Processing: false,
         ...
         try {
           ...
           this.props.SetLoading(false)
             ...
             data.forEach(element => {
                if(element.key === 'TherapeuticSessions'){
   ...
   getConsultingResult(){
      NetInfo.fetch().then(state =>{
        if (state.isConnected){
          try {
            axios.get('http://68.183.39.113/api/user/getUserRequests',{
               params:{
        ...
        return(
            ...
            <View style={[{flex: 1,paddingHorizontal: 24,alignItems:'center' }]}>
                 ...
                 <Text style={[this.props.Language == "AR" ? styles.right :styles.left,{ color: '#707070', fontSize: 20,fontFamily:'adobe',
                   ...
                   {this.props.Language == "AR" ? "ما هى الجلسات العلاجية ؟" : "What is awareness lecture"}
                 </Text>
                 <Text style={[this.props.Language == "AR" ? styles.right :styles.left,{ color: '#707070', fontSize: 20,fontFamily:'adobe',
            ...
            {/* <View style={[styles.view,{position:'absolute',bottom:60}]}>
            ...
            <Text style={{ width: '100%',textAlign:'center',alignItems:'center',color:'#707070', fontSize:18,fontFamily:'adobe',marginTop:5}}>
               ...
               <LinearGradient colors={['#47D9D3', '#3D7371']} style={[styles.button,styles.shadow,]}>
                  ...
                  onPress={async () => { 
                     if(this.props.User.loginType ===1){
                        const result = await AsyncStorage.getItem('ConsultingResult');
                         if(result){
                             if(result==1){
                              this.setState({ isVisible: true })
                             }else{
                              this.props.navigation.navigate('TherapeuticSessions2')
                             }
                         }else{
...
const mapStateToProps = state => {
 ...
 // redux
 export default connect(mapStateToProps,{ SetLoading})(TherapeuticSessions)

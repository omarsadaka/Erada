...
class StutterControl extends Component{
    constructor(props) {
        super(props);
        this.state = {
            Processing: false,
    ...
    onCreate=()=>{
       const{ input , filePath } = this.state
       if(input){
          if(filePath){
            this.props.SetLoading(true)
    ...
    Register() {
     ...
     const{ input , filePath } = this.state
                 if( input  ){
                     if( filePath){
                       this.props.UserRegister(username, mobile, email, password , 2 , type , input , filePath)
                 }else{
                    if(this.props.Language=='AR'){
                  ...
                  if(this.props.Language=='AR'){
                     alert("اكتب نبذة عن الخبرات السابقة")
                  }else{
                     alert("Enter your experience")
                  }
                 }
        ...
        if (lang == "AR") {
           return (
              <View style={[styles.flex, styles.row, styles.shadow, { width: width, height: 55, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#28B5AF', paddingHorizontal: 18 }]} >
                 ...
                 <Text style={{ color: '#fff', fontSize: 22,fontFamily:'adobe' }}>متحكم فى التأتأة</Text>
                 <View></View>
              </View>
           )
        } else {
           return (
              <View style={[styles.flex, styles.row, styles.shadow, { width: width, height: 55, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#28B5AF', paddingHorizontal: 18 }]} >
                 <View></View>
                 <Text style={{ color: '#fff', fontSize: 22,fontFamily:'adobe' }}>Stutter control</Text>
                 <TouchableOpacity onPress={() => {
                     if(this.state.chechBack === 1){
                     this.props.navigation.navigate('Register')
                    }else{
               ...
               <View style={[styles.row, { justifyContent: 'center', alignItems: 'center' }]}>
                  <Item style={[styles.inputFields, styles.shadow, { }]}>
                     <Input
                      ...
                      style={[this.props.Language == "AR" ? styles.right :styles.left ,{ color: '#764c22',fontSize:14,width:'95%',height:120,textAlignVertical:'top' }]}
...
const mapStateToProps = state => {
 ...
 // redux
 export default connect(mapStateToProps,{ SetLoading , UserRegister})(StutterControl)
